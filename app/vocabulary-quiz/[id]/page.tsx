...
export default function VocabularyQuizPage() {
  ...
  const [quizData, setQuizData] = useState<VocabularyQuizData | null>(null)
  ...
  const fetchQuiz = useCallback(async () => {
    try {
      const response = await fetch(`/api/vocabulary-quiz/${quizId}`)
      if (response.ok) {
  ...
  const handleQuizComplete = useCallback(
    async (score: number, userAnswers: Record<number, string>) => {
      try {
        const response = await fetch("/api/vocabulary-quiz/submit", {
          method: "POST",
          headers: {
    ...
    return (
      <div className="container mx-auto p-4 md:p-8 max-w-4xl">
        <div className="flex items-center justify-center min-h-[400px]">
          <LoadingSpinner size="lg" text="Loading vocabulary quiz..." />
        </div>
      </div>
    )
  }
      ...
      <div className="container mx-auto p-4 md:p-8 max-w-4xl">
        <div className="text-center space-y-6 min-h-[400px] flex flex-col justify-center">
          ...
          <p className="text-muted-foreground">
            Please sign in to access this vocabulary quiz.
          </p>
    ...
    return (
      <div className="container mx-auto p-4 md:p-8 max-w-4xl">
        <div className="text-center space-y-6 min-h-[400px] flex flex-col justify-center">
          ...
          <p className="text-muted-foreground">
            {error ||
              "The vocabulary quiz you're looking for doesn't exist or you don't have access to it."}
          </p>
